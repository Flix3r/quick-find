...
use tauri::{
    Emitter, Manager, LogicalPosition, LogicalSize, AppHandle, 
...
fn open(app: &AppHandle, menu: &mut Menu) {
    ...
    let window = app.get_window("main")
        .expect("Could not get app window");
    let webview = app.get_webview("main")
        .expect("Could not get app webview");
    let cursor_pos = app.cursor_position()
        .expect("Could not get cursor position");
    let cursor_monitor = app.monitor_from_point(cursor_pos.x, cursor_pos.y)
        .expect("Could not get monitor at cursor").expect("Could not find monitor at cursor");
    
    if !window.is_visible().expect("Could not check if window is visible") {
        let monitor_pos = cursor_monitor.position();
        let monitor_size = cursor_monitor.size();

        window.set_position(LogicalPosition::<i32>::new(
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            TrayIconBuilder::new()
                ...
                .menu(&ContextMenu::with_items(app, &[
                ...
                tauri_plugin_global_shortcut::Builder::new()
                    .with_handler(|app, shortcut, event| {
                        if event.state == ShortcutState::Pressed { 
                            for (index, menu) in app.state::<Mutex<Vec<Menu>>>().lock().unwrap().iter_mut().enumerate() {
